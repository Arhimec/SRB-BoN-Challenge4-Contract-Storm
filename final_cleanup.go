...
func main() {
	if len(os.Args) < 3 { log.Fatal("Usage: final_cleanup <master_addr> <active_99_file>") }
	master, activeFile := os.Args[1], os.Args[2]
	proxy := "https://gateway.battleofnodes.com"
	
	// Target exactly 5.0 EGLD for the 99 wallets.
	targetBal, _ := new(big.Int).SetString("5000000000000000000", 10)
	fee := big.NewInt(100000000000000) // 0.0001 reserve for gas
	...
	for _, dir := range wallets {
		...
		for _, f := range files {
			...
			go func(p ed25519.PrivateKey, a string) {
				...
				if activeMap[a] {
					// 99 Active Wallets: Cap at 5.0 EGLD
					if bal.Cmp(new(big.Int).Add(targetBal, fee)) > 0 {
						toSend = new(big.Int).Sub(bal, targetBal)
						toSend.Sub(toSend, fee)
						fmt.Printf("[%s] Active: Capping to 5.0 EGLD (Sending %s excess)\n", a, toSend.String())
					}
				} else {
					// 15 Excess Wallets: Drain to 0
					if bal.Cmp(fee) > 0 {
						toSend = new(big.Int).Sub(bal, fee)
						fmt.Printf("[%s] Excess: Draining to 0 (Sending %s)\n", a, toSend.String())
					}
				}

				if toSend != nil && toSend.Cmp(big.NewInt(0)) > 0 {
					tx := &Transaction{
