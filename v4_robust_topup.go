...
func main() {
	...
	fmt.Printf("Starting ROBUST top-up from master %s\n", masterAddr)
	...
	for _, t := range targets {
		...
		if bal.Cmp(targetBal) < 0 {
			...
			for retry := 0; retry < 5; retry++ {
				mNonce := getNonce(proxy, masterAddr)
				tx := &Transaction{
					Nonce: mNonce, Value: diff.String(), Receiver: t, Sender: masterAddr,
					GasPrice: 2000000000, GasLimit: 50000, ChainID: "B", Version: 1,
				}
				if err := send(proxy, tx, priv); err == nil {
					fmt.Printf("[%s] Broadcast SUCCESS (Nonce %d). Waiting for sync...\n", t, mNonce)
					
					// Verify nonce increase
					synced := false
					for wait := 0; wait < 30; wait++ {
						time.Sleep(2 * time.Second)
						newNonce := getNonce(proxy, masterAddr)
						if newNonce > mNonce {
							fmt.Printf("[%s] VERIFIED: Nonce increased to %d\n", t, newNonce)
							synced = true
							break
						}
					}
					if synced { break }
					fmt.Printf("[%s] WARNING: Sync timeout, retrying...\n", t)
				} else {
					fmt.Printf("[%s] ERROR: %v. Retrying in 5s...\n", t, err)
    ...
    fmt.Println("Robust stabilization complete.")
}
