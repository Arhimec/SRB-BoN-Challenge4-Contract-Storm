...
func main() {
	if len(os.Args) < 3 { log.Fatal("Usage: drain <master_addr> <final_99_file>") }
	masterAddr, final99File := os.Args[1], os.Args[2]
				...
				go func(privKey ed25519.PrivateKey, addr string) {
					...
					if bal.Cmp(fee) > 0 {
						...
						if err := send(proxy, tx, privKey); err == nil {
							fmt.Printf("[%s] DRAINED extra wallet (Value: %s)\n", addr, d.String())
						} else {
							fmt.Printf("[%s] DRAIN ERROR: %v\n", addr, err)
						}
					}
				}(p, a)
